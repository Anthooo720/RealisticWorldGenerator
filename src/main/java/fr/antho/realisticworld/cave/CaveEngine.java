...
/**
 * Surcouche vanilla+ : les carvers Minecraft restent responsables du réseau principal.
 * RWG ajoute des connecteurs, salles rares et surtout une irrégularité de contour qui évite
 * les tubes lisses. Les détails de surface sont déterministes et rendus ensuite par le générateur.
 */
public final class CaveEngine {
    ...
    public boolean shouldCarve(GeologyMap.GeologySample geo,int x,int y,int z,double surface) {
        ...
        double detailScale=Math.max(0.012,cfg.detailScale());
        double detail=roughness.fbm(sx*detailScale,sy*detailScale*0.82,sz*detailScale,3,2.02,0.52);
        double detailRadius=1.0+detail*MathUtil.clamp(cfg.surfaceDetailStrength(),0.0,0.75)*0.34;
        double baseRadius=Math.max(0.024,cfg.tunnelRadius()*1.05)*geoRadius
                *(0.92+depth*0.14)*(0.82+MathUtil.clamp(cfg.overlayStrength(),0.35,1.40)*0.36)*detailRadius;

        boolean connector=Math.hypot(a,b)<baseRadius;
        boolean branch=Math.hypot(c,d)<baseRadius*0.64 && mask>0.48;
        ...
        boolean chamber=isInsideRareRoom(x,y,z,depth,geoRadius)
                && (Math.hypot(a,b)<baseRadius*3.0 || Math.hypot(c,d)<baseRadius*2.4);
    ...
    private boolean isInsideRareRoom(int x,int y,int z,double depth,double geoRadius) {
        ...
        for(int dz=-1;dz<=1;dz++) for(int dy=-1;dy<=1;dy++) for(int dx=-1;dx<=1;dx++) {
            ...
            double maxR=MathUtil.clamp(cfg.maxChamberRadius(),5.0,12.0);
            double radius=(4.4+HashUtil.unitDouble(HashUtil.mix64(h^0x44L))*(maxR-4.4))*geoRadius;
            radius*=0.95+depth*0.12;
            double nx=(x-px)/radius, ny=(y-py)/(radius*0.60), nz=(z-pz)/radius;
            double wobble=roughness.sample(x*0.055,y*0.050,z*0.055)*0.13;
            if(nx*nx+ny*ny+nz*nz<0.74+wobble) return true;
        }
        return false;
    }

    /** Décisions de décoration très rares, stables et sans cache supplémentaire. */
    public DetailSample detailSample(int x,int y,int z) {
        long h=hash3(x,y,z,0x1700DE7A1L);
        boolean natural=HashUtil.unitDouble(h)<MathUtil.clamp(cfg.decorationFrequency(),0.0,0.12);
        boolean step=HashUtil.unitDouble(HashUtil.mix64(h^0x51EFL))<MathUtil.clamp(cfg.stepFrequency(),0.0,0.06);
        boolean slab=(HashUtil.mix64(h^0x5AABL)&1L)==0L;
        int facing=(int)Math.floorMod(HashUtil.mix64(h^0xFACEL),4L);
        return new DetailSample(natural,step,slab,facing);
    }
    ...
    public record DetailSample(boolean naturalDecoration,boolean rockStep,boolean slab,int facingIndex) {}
}
