...
public final class WaterColumnEngine {
    ...
    /**
     * Pente de surface calculée depuis les colonnes déjà cachées. Les anciennes passes sol,
     * végétation et détails appelaient chacune TerrainEngine.slope(), soit quatre nouvelles
     * évaluations de terrain par appel. Ici les mêmes hauteurs servent à toutes les passes.
     */
    public double slope(int x,int z) {
        final int d=3;
        double dx=sample(x+d,z).naturalHeight()-sample(x-d,z).naturalHeight();
        double dz=sample(x,z+d).naturalHeight()-sample(x,z-d).naturalHeight();
        return Math.hypot(dx,dz)/(2.0*d);
    }
